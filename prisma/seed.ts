...
async function main() {
  ...
  // ========== Contractors ==========
  const contractor1 = await db.contractor.create({ data: { name: 'عبدالله المنصوري', nameEn: 'Abdullah Al Mansouri', companyName: 'شركة المنصوري للمقاولات', companyEn: 'Al Mansouri Contracting', contactPerson: 'عبدالله المنصوري', phone: '+971-50-999-1122', email: 'info@almansouri-contract.ae', address: 'رأس الخيمة', crNumber: 'RAK-2024-CR001', licenseNumber: 'RAK-LIC-2024-001', category: 'civil', rating: 5, specialties: 'تشطيبات,أشغال مدنية', experience: '15 سنة في مجال المقاولات', bankName: 'مصرف أبوظبي التجاري', bankAccount: 'AE07 0331 2345 6789 0123 456', iban: 'AE07 0331 2345 6789 0123 456', isActive: true } });
  const contractor2 = await db.contractor.create({ data: { name: 'سالم الكعبي', nameEn: 'Salem Al Kaabi', companyName: 'مجموعة الكعبي للإنشاءات', companyEn: 'Al Kaabi Construction Group', contactPerson: 'سالم الكعبي', phone: '+971-50-888-3344', email: 'projects@alkaabi-build.ae', address: 'دبي', crNumber: 'DXB-2024-CR002', licenseNumber: 'DXB-LIC-2024-002', category: 'civil', rating: 4, specialties: 'مباني,إنشاءات خرسانية', experience: '20 سنة في المشاريع الكبرى', bankName: 'بنك الإمارات دبي الوطني', bankAccount: 'AE03 0331 9876 5432 1098 765', iban: 'AE03 0331 9876 5432 1098 765', isActive: true } });
  const contractor3 = await db.contractor.create({ data: { name: 'راشد الشحي', nameEn: 'Rashid Al Shamsi', companyName: 'شركة الخليج للأعمال الكهربائية', companyEn: 'Gulf Electrical Works', contactPerson: 'راشد الشحي', phone: '+971-50-777-5566', email: 'info@gulf-electrical.ae', address: 'أبو ظبي', crNumber: 'ADH-2024-CR003', licenseNumber: 'ADH-LIC-2024-003', category: 'electrical', rating: 4, specialties: 'كهرباء,تكييف,سباكة', experience: '10 سنوات في الأنظمة الكهروميكانيكية', bankName: 'بنك أبوظبي الأول', bankAccount: 'AE12 0331 5555 6666 7777 888', iban: 'AE12 0331 5555 6666 7777 888', isActive: true } });
  const contractor4 = await db.contractor.create({ data: { name: 'حمد المعلا', nameEn: 'Hamad Al Mualla', companyName: 'شركة المعلا للتشطيبات الفاخرة', companyEn: 'Al Mualla Luxury Finishing', contactPerson: 'حمد المعلا', phone: '+971-50-666-7788', email: 'info@almualla-finishing.ae', address: 'رأس الخيمة', crNumber: 'RAK-2024-CR004', licenseNumber: 'RAK-LIC-2024-004', category: 'finishing', rating: 5, specialties: 'تشطيبات فاخرة,رخام,دهانات', experience: '12 سنة في التشطيبات الداخلية والخارجية', bankName: 'مصرف رأس الخيمة', bankAccount: 'AE21 0331 1111 2222 3333 444', iban: 'AE21 0331 1111 2222 3333 444', isActive: true } });
  console.log('✅ 4 demo contractors created');

  // ========== Projects ==========
  const project1 = await db.project.upsert({ where: { number: 'PRJ-2024-001' }, update: {}, create: { number: 'PRJ-2024-001', name: 'فيلا فاخرة - المنطقة الأولى', nameEn: 'Luxury Villa', clientId: client1.id, contractorId: contractor1.id, location: 'دبي', type: 'villa', status: 'active', progress: 65, budget: 250000, startDate: new Date('2024-01-15'), endDate: new Date('2024-12-30'), description: 'تصميم وبناء فيلا فاخرة', createdById: adminUser.id } });
  const project2 = await db.project.upsert({ where: { number: 'PRJ-2024-002' }, update: {}, create: { number: 'PRJ-2024-002', name: 'مبنى سكني متعدد الطوابق', nameEn: 'Residential Building', clientId: client2.id, contractorId: contractor2.id, location: 'أبو ظبي', type: 'building', status: 'active', progress: 40, budget: 850000, startDate: new Date('2024-03-01'), endDate: new Date('2025-06-30'), description: 'مبنى سكني 12 طابق', createdById: pmUser.id } });
  const project3 = await db.project.upsert({ where: { number: 'PRJ-2024-003' }, update: {}, create: { number: 'PRJ-2024-003', name: 'مجمع تجاري - المنطقة الحرة', nameEn: 'Commercial Complex', clientId: client3.id, contractorId: contractor3.id, location: 'رأس الخيمة', type: 'commercial', status: 'active', progress: 20, budget: 1200000, startDate: new Date('2024-06-01'), endDate: new Date('2025-12-31'), description: 'مجمع تجاري متعدد الاستخدامات', createdById: pmUser.id } });
  const project4 = await db.project.upsert({ where: { number: 'PRJ-2024-004' }, update: {}, create: { number: 'PRJ-2024-004', name: 'فيلا عائلية', nameEn: 'Family Villa', clientId: client4.id, contractorId: contractor4.id, location: 'الشارقة', type: 'villa', status: 'completed', progress: 100, budget: 180000, startDate: new Date('2023-06-01'), endDate: new Date('2024-03-15'), description: 'فيلا عائلية من طابق واحد', createdById: adminUser.id } });
  ...
  await db.bid.createMany({
    data: [
      { projectId: project1.id, contractorId: contractor1.id, contractorName: contractor1.companyName, amount: 220000, technicalScore: 85, financialScore: 90, totalScore: 87.5, status: 'accepted', deadline: new Date('2024-01-10') },
      { projectId: project1.id, contractorId: contractor4.id, contractorName: contractor4.companyName, amount: 245000, technicalScore: 90, financialScore: 75, totalScore: 82.5, status: 'rejected', deadline: new Date('2024-01-10') },
      { projectId: project2.id, contractorId: contractor2.id, contractorName: contractor2.companyName, amount: 780000, technicalScore: 88, financialScore: 85, totalScore: 86.5, status: 'accepted', deadline: new Date('2024-02-20') },
      { projectId: project3.id, contractorId: contractor3.id, contractorName: contractor3.companyName, amount: 1100000, technicalScore: 82, financialScore: 88, totalScore: 85, status: 'accepted', deadline: new Date('2024-05-15') },
      { projectId: project5.id, contractorId: contractor1.id, contractorName: contractor1.companyName, amount: 550000, technicalScore: 80, financialScore: 82, totalScore: 81, status: 'submitted', deadline: new Date('2024-02-10') },
      { projectId: project5.id, contractorId: contractor2.id, contractorName: contractor2.companyName, amount: 580000, technicalScore: 85, financialScore: 78, totalScore: 81.5, status: 'submitted', deadline: new Date('2024-02-10') },
    ],
  });
  console.log('✅ Bids with contractors seeded');
